...
Stepper myStepperY(PassosPorVolta, 6, 7, 8, 9, true);  
Stepper myStepperX(PassosPorVolta, A0,A1,4,5, true);

Stepper myStepperUPY(PassosPorVolta, 6, 7, 8, 9, true);  
Stepper myStepperUPX(PassosPorVolta, A0,A1,4,5, true);
...
const float StepInc = 1.0;
const int StepDelay = 0;
const int LineDelay = 0; // delay para dar tempo ao código em java receber "Ok" e enviar a próxima linha.
...
const int penZUp = 20.0;
const int penZDown = -20.0;

const float StepsPerMillimeterX = 15.4; 
const float StepsPerMillimeterY = 15.4;

// Valores em milímetros do limite da área útil de impressão:
const float Xmin = 0.00, Xmax = 90.00;
const float Ymin = 0.00, Ymax = 85.00;
...
void setup() {
  ...
  myStepperUPX.setSpeed(350); 
  myStepperUPY.setSpeed(350);
...
void loop() {
  ...
  int lineIndex = 0;                     // Índice para controlar a posição do vetor 'line'.
  bool lineIsComment = false;                // Flag para identificar e ignorar comentários e ';' nas linhas recebidas.

  while (1) {

    while ( Serial.available()>0 ) {   // Enquanto tiver dados para receber na porta serial, esse laço vai gravar esses dados em line[ LINE_BUFFER_LENGTH ].
...
/*  
    ...
    este laço vai verificar cada caractere novamente e executar seus respectivos comandos.
*/
...
void drawLine(float x1, float y1) { 
  ...
  y1 = (y1 >= Ymax) ? Ymax : (y1 <= Ymin) ? Ymin : y1;
...
// Variáveis auxiliares para o algorítimo de Bresenham:
  ...
  if(caneta) {
    ...
    // Algoritmo de Bresenham (linha horizontal maior que vertical):
      if (dx > dy) {
        for (i=0; i<dx; ++i) {
          myStepperUPX.step(sx);
          over+=dy;
          if (over>=dx) {
            over-=dx;
            myStepperUPY.step(sy);
        }
          delay(StepDelay);
        }
      }
