...
Stepper StepperZ(PassosPorVolta, 10, 11, 12, 13);
...
void setup() {
  ...
  StepperX.setSpeed(drawSpeed);
  StepperY.setSpeed(drawSpeed);
  StepperZ.setSpeed(moveSpeed);

  StepperXlevantado.setSpeed(moveSpeed);
  StepperYlevantado.setSpeed(moveSpeed);

  //limitStep, mult
  StepperX.         setAcel(30, 3);
  StepperXlevantado.setAcel(100, 2);
...
void drawLine(float targetX, float targetY) {
  ...
  Stepper &motorX = caneta ? StepperXlevantado : StepperX;
  Stepper &motorY = caneta ? StepperYlevantado : StepperY;

  long over = 0;
  int corrigido = 0;
  int n = (disX > disY) ? disX : disY;  // Número de iterações = maior entre disX e disY

  motorX.resetAcelParameters(n);
