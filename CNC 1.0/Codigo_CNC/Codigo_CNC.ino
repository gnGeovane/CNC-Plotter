...
Stepper myStepperZ(PassosPorVolta, 10,11,12,13);            
Stepper myStepperY(PassosPorVolta, 9,8,7,6);  
Stepper myStepperX(PassosPorVolta, 2,3,4,5);
...
while (currentIndex < charNB) { 
    ...
    case 'M':
      ...
      case 8: // Parada para esfriar motores
        ...
        for(int i = Tempo_Resfriamento; i > 0; i--) {
...
void drawLine(float x1, float y1) { 
  ...
  long dx = abs(x1-x0); // abs = valor absoluto, ou simplesmente Módulo
  long dy = abs(y1-y0);
